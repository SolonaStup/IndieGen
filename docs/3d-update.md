...
paid the same way (per generation, in $INDIEGEN). This doc covers the vision,
...
Bring the INDIEGEN promise — *prompt in, game asset out* — to the third
...
[how-it-works.md](how-it-works.md)) — sign-in, live price, burn 50% / treasury
50%, pay-on-success — with one difference: **price scales with complexity.**
...
- A 3D generation is priced **$2–$15 in USD**, estimated from the prompt
  ...
  detailed…*).
- That USD price is converted to $INDIEGEN at the **live market rate** and paid
  from the wallet, **50% burned 🔥**.

```
simple prop      →  ~$2   →  burn 50% + treasury 50%
rigged character →  ~$15  →  burn 50% + treasury 50%
```

Because 3D generations cost more than 2D, **each one burns more $INDIEGEN** —
...
> leaves healthy margin to cover compute and fund the treasury.
...
engine-ready 3D models — priced $2–$15 by complexity, paid in $INDIEGEN with
half of every payment burned, making it the platform's biggest deflation engine.
